...
![image-20201124142601445](第6次作业.assets/image-20201124142601445.png)

## 解答

1. 存储结构：邻接矩阵，矩阵中存放`double`，代表两种货币的换算指数。
2. 输入数据：货币种数、汇率换算次数、汇率换算详情
3. 输出数据：`Yes`-可以套汇；`No`-不可以套汇
4. 时间复杂度：三重循环，应该是$O(n^3)$

**解题思路：**

1. 初始化一个邻接矩阵
2. 循环每一个顶点，检查是否有可能有套汇的可能
   1. 初始化遍历的参数、访问位
   2. 以该顶点为出发点进行DFS
   3. 如果有成环且系数大于1则返回Yes
...
![image-20201129220922061](第6次作业.assets/image-20201129220922061.png)
